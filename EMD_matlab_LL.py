...
#
# Edited:   11/05/2017
# Edited by L. lancia at line 432 on the 08/09/2024 to solve compatibility issues with numpy>=2
...
class EMD:
    ...
    *Note:*
    ...
    def _common_dtype(x, y):
        dtype = np.promote_types(x.dtype, y.dtype) #dtype = np.find_common_type([x.dtype, y.dtype], [])
        if x.dtype != dtype:
            x = x.astype(dtype)
        if y.dtype != dtype:
            y = y.astype(dtype)
