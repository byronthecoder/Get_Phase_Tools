...
% should be equivalent to running one sifting iteration from EMD
% emdComps=emd(signal,'Display',0,'SiftMaxIterations',1,'MaxNumIMF',1);%run EMD
...
% PHI=quadAngle(normSig);

PHI=angle(hilbert(normSig,tmpLen));%hilbert phase (the first estimate is used to infer local frequancy)
PHI=PHI(1:origLen);

PHIc=correct_PHI(PHI,SG_coef,handleNegPHI);%corrected phase

%estimation of local frequancies
nPts=(length(signal)-(wLenFr/2))/wStepFr; %total number of estimates
Ts=winLen/2+[1:nPts].*winStep; % Time points around which estimation windows are centered

allFreqs=zeros(length(Ts),1); % initialize vector of local frequencies
for i=1:nPts
    startPt=(i-1)*wStepFr+1; % window atart pt.
    endPt=startPt+wLenFr-1; % window endpt
    endPt=min([endPt,length(signal)]); %correct if later than signal length
    if endPt<=startPt
        continue
    end
    thisChunk=signal(startPt:endPt); % get signal chunk
    thisPhChunk=PHIc(startPt:endPt); % get phase chunk
    thisPHIu=unwrap(thisPhChunk); % unwrap phase
    %get average frequency
    allFreqs(i)=(thisPHIu(end)-thisPHIu(1))/(2*pi*length(thisChunk));
end
t=[1:length(PHI)]./sr; %time stamps of the phase signal
timeVarFreq=interp1(Ts,allFreqs,t,'pchip','extrap'); % produce continuously changing frequency signal
mask=sin(wrapToPi(2*pi*cumsum(timeVarFreq))); % synthetize synewave with changing freq.
...
newPHI=quadAngle(IMF); % quadrature phase estimation (here we need to be unbiased so we may not want to use Hilbert)
